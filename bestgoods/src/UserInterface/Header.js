...
export default function Header(props) {
  ...
  const list = (anchor) => (
    ...
    >
      ...
      <div style={{padding:5,display:'flex',alignItems:'center',width:390}}>
        <Button onClick={handleProceed} variant="contained" fullWidth style={{background:'black',color:'#FFF',fontWeight:'bold',fontSize:16}}>Proceed</Button>
      </div>
    </Box>
  );
  /****************/
          ...
          {listSubCategory.map((item) => {
            return (
              <MenuItem onClick={() => handleMenuClick(item.subcategoryid)}>
                <img src={`${ServerURL}/images/${item.picture}`} width="20" style={{ marginRight: 10 }} />
  ...
  const mainMenu = () => {
    return categoryList.map((item, index) => {
      ...
      return (index <= 3 ? <Button value={item.categoryid} onClick={handlemClick} style={{ color: '#000', fontWeight: 'bold', marginLeft: 30, fontSize: '18' }}>{item.categoryname}</Button> : <></>)
    })
  }
