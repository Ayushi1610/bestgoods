...
export default function SideBar(props) {
    ...
    const [price,setPrice]=useState({min:1000,max:100000})
    ...
    const handleChange1 = (event, newValue, activeThumb) => {
        ...
        setPrice({min:[Math.min(newValue[0], value1[1] - minDistance), value1[1]],max:[value1[0], Math.max(newValue[1], value1[0] + minDistance)]})
    };
    const handleClick = () => {
        setValue(!value)
        setState(!state)
    ...
    return (
        ...
        <div style={{position:'-webkit-sticky',position:'sticky',top:100,bottom:0,left:10,right:0,zIndex:5}}>
