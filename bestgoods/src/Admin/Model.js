  ...
  export default function Models(props)
{
    ...
    const handlePicture=(event)=>{
      ...
      const handleSubmit=async()=>{
        ...
        var result= await postDataAndImage('model/modelsubmit',formData,{headers:{"content-type":'multipart/formData'}})
        if(result)
        {
        Swal.fire({
       ...
       <div className={classes.subdiv}>
         ...
         <Grid item xs={12}>
           ...
           <div style={{display:'flex',flexDirection:'row'}}>
             ...
             <div>
               <Button onClick={()=>handleClick()} variant="contained" startIcon={<List/>}>Models List</Button>
              </div>
              </Grid>
           </Grid>
          </Grid>
        ...
        >
          {fillCompany()}
            ...
            <Grid item xs={6}>
              <TextField onChange={(event)=>setSize(event.target.value)} fullWidth variant="outlined" label="Size"/>
            </Grid>
            <Grid item xs={6}>
              <TextField onChange={(event)=>setWeight(event.target.value)} fullWidth variant="outlined" label="Weight"/>
            </Grid>
            <Grid item xs={6}>
